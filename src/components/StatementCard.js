        ...
        return (
            <div className="comment">
                <p className="col-md-12" style={{border}}>
                    ...
                    {text} <br/>
                    { showChallenge &&(<button type="button" className="full-debate challenge" href="#" data-toggle="modal" data-target="#challenge-conf" aria-hidden="true" onClick={ () => handleChallenge(_id, topic._id)}><b> CHALLENGE! &#8250;</b></button>)}
                </p>
                <div className="col-md-6 button-container">
                    <button className={ "button-vote up " + ((loggedInUsersVote && loggedInUsersVote.isRational) ? "clicked" : "") } onClick={ () => handleVote(true, _id) }><img className="factual-vote" src="images/best-debater-w.png"/> &nbsp; <span className="vote-num">{ voters.filter(v => v.isRational).length }</span></button>
                </div>
                <div className="col-md-6 button-container">
                    <button className={ "button-vote down " + ((loggedInUsersVote && !loggedInUsersVote.isRational) ? "clicked" : "") } onClick={ () => handleVote(false, _id) }><img className="emotional-vote" src="images/heart-w.gif" />  &nbsp; <span className="vote-num">{ voters.filter(v => !v.isRational).length }</span></button>
                </div>
            </div>
        );
    };
